...
def initImage(imagefile):   
    imhdu = fits.open(imagefile)
    imdata = imhdu[0].data
    #blank = np.zeros_like(imdata)
...
def log_prior(theta, **kwargs):
    amp1, reff1, n1, amp2, reff2, n2 = theta #BDW
...
def convolved(theta, **kwargs): 
    ...
    amp1, reff1, n1, amp2, reff2, n2 = theta
    ...
    sersic2 = Sersic2D(amplitude=amp2, r_eff=reff2, n=n2, x_0=xs-5, y_0=ys-5)
    ...
    #S1 = sersic1(xsscut, ysscut)#.value
    #S2 = sersic2(xsscut, ysscut)#.value
    S1S2 = sersic1(xsscut, ysscut) + sersic2(xsscut, ysscut)
